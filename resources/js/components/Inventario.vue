<template>
  <section class="inventory-card">
    <div class="inventory-hero">
      ...
      <button
        type="button"
        class="inventory-kardex-toggle"
        :class="{ active: vista === 'kardex' }"
        @click="vista = vista === 'kardex' ? 'stock' : 'kardex'"
      >
        {{ vista === 'kardex' ? '← Stock actual' : 'Ver movimientos / Kardex' }}
      </button>
    </div>

    <div v-if="vista === 'stock'" class="inventory-body">
      ...
      <div class="filters-panel">
        ...
        <div class="filter-grid">
          ...
          <div class="filter-actions">
            <button type="button" class="btn-search" @click="cargarInventario">
              Buscar
            </button>
            ...
            <template v-else v-for="producto in productos" :key="producto.id">
              <tr class="product-row" :class="{ expanded: expandido === producto.id }">
                <td>
                  ...
                  <small>{{ producto.tipo_producto ? tipoTexto(producto.tipo_producto) : '' }}</small>
                </td>
                <td class="code-cell">{{ producto.codigo }}</td>
                <td>
                  <strong>{{ producto.partida?.codigo || '—' }}</strong>
                </td>
                <td>
                  <span class="group-text">{{ producto.grupo_producto || '—' }}</span>
                </td>
                <td>
                ...
                <td>
                  ...
                  <small v-if="producto.estado_vencimiento === 'PROXIMO'" class="date-warning">
                    Próximo a vencer
                  </small>
                  <small v-if="producto.estado_vencimiento === 'VENCIDO'" class="date-danger">
                    Vencido
                  </small>
                </td>
                <td>
                  <span class="status-pill" :class="estadoClass(producto.estado_stock)">
                    {{ estadoTexto(producto.estado_stock) }}
                  </span>
                </td>
                <td class="text-center">
                  <button
                    type="button"
                    class="detail-button"
                    @click="toggleDetalle(producto.id)"
                  >
    ...
    <div v-else class="inventory-body">
      <div class="kardex-title">
        <div>
          <h3>Kardex de ingresos</h3>
          <p>Historial de los ingresos registrados al almacén.</p>
        </div>
      </div>

      <div class="filter-grid kardex-filters">
        <label>
          Producto / LINAME
          <input v-model="kardexFiltros.buscar" class="form-control" placeholder="Producto, concentración o código">
        </label>
        <label>
          Procedencia / proveedor
          <input v-model="kardexFiltros.procedencia" class="form-control" placeholder="Proveedor">
        </label>
        <label>
          Desde
          <input v-model="kardexFiltros.fecha_desde" type="date" class="form-control">
        </label>
        <label>
          Hasta
          <input v-model="kardexFiltros.fecha_hasta" type="date" class="form-control">
        </label>
        <div class="filter-actions">
          <button type="button" class="btn-search" @click="cargarKardex">Buscar</button>
          <button type="button" class="btn-clear" @click="limpiarKardex">Limpiar</button>
        </div>
      </div>

      <div class="table-wrap">
        <table class="inventory-table kardex-table">
          <thead>
            <tr>
              <th>Fecha</th>
              <th>Nota / remisión</th>
          ...
          <tbody>
            <tr v-if="cargandoKardex">
              <td colspan="11" class="empty-row">Cargando kardex…</td>
            </tr>
            <tr v-else-if="kardex.length === 0">
              <td colspan="11" class="empty-row">No se encontraron ingresos con esos filtros.</td>
            </tr>
...
const partidas = computed(() => {
  ...
  catalogoCompleto.value.forEach(producto => {
    if (producto.partida?.codigo) {
      mapa.set(producto.partida.codigo, producto.partida);
    }
  });
...
const tipos = computed(() =>
  [...new Set(catalogoCompleto.value.map(p => p.tipo_producto).filter(Boolean))].sort()
);
...
const cargarInventario = async () => {
  ...
  try {
    ...
    // Obtenemos el catálogo completo una sola vez para alimentar los filtros
    // y los indicadores sin depender de la búsqueda actual.
    if (catalogoCompleto.value.length === 0 && Object.values(filtros).some(Boolean)) {
      const completo = await axios.get('api/inventario');
      catalogoCompleto.value = completo.data;
    } else if (catalogoCompleto.value.length === 0) {
      catalogoCompleto.value = data;
    }
  } catch (e) {
...
const cargarKardex = async () => {
  ...
  try {
    const { data } = await axios.get('api/kardex', { params: kardexFiltros });
    kardex.value = data;
  } finally {
...
const limpiarKardex = () => {
  Object.keys(kardexFiltros).forEach(key => { kardexFiltros[key] = ''; });
  cargarKardex();
};
...
.inventory-card {
  ...
  border-radius: 14px;
  overflow: hidden;
  box-shadow: 0 5px 20px rgba(20, 48, 70, .07);
}
...
.inventory-kardex-toggle:hover,
.inventory-kardex-toggle.active {
  background: #e85d04;
  border-color: #e85d04;
}
...
.summary-warning { border-left-color: #e85d04; }
.summary-danger { border-left-color: #b42318; }
...
.filter-grid {
  display: grid;
  grid-template-columns: 2fr 1.4fr 1.25fr 1.25fr 1.45fr auto;
  gap: 10px;
  align-items: end;
}
