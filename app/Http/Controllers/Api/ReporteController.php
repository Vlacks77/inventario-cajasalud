...
class ReporteController extends Controller
{
    ...
    public function salidaPdf(Salida $salida) {
        ...
        return Pdf::loadView('pdf.nota-salida', compact('salida'))->setPaper('letter','landscape')->download('nota-salida-'.$salida->numero_salida.'.pdf', ['Content-Type' => 'application/pdf']);
    }
    public function salidaExcel(Salida $salida) { $salida->load(['establecimiento','usuario','detalles.lote.medicamento.partidaPresupuestaria']); return $this->excel('Reporte de salida '.$salida->numero_salida, view('excel.salida',compact('salida'))->render(), 'nota-salida-'.$salida->numero_salida.'.xls'); }
    public function inventario(Request $request) {
        $data=$this->inventarioData($request); return Pdf::loadView('pdf.inventario',['productos'=>$data,'filtros'=>$request->all(),'fecha'=>now()])->setPaper('letter','landscape')->download('reporte-inventario-'.now()->format('Ymd-His').'.pdf', ['Content-Type' => 'application/pdf']);
    }
    ...
    public function kardex(Request $request) {
        ...
        usort($mov,fn($a,$b)=>strcmp($a['fecha'],$b['fecha'])); $stock=0; foreach($mov as &$m){$stock += $m['entrada']-$m['salida'];$m['stock']=$stock;} return Pdf::loadView('pdf.kardex',compact('producto','mov','desde','hasta'))->setPaper('letter','landscape')->download('kardex-'.$producto->codigo.'.pdf', ['Content-Type' => 'application/pdf']);
    }
